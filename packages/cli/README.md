...
`skill` copies the packaged skills into `.claude/skills/ctw-native` and `.claude/skills/ctw-native-check`. `plugin-zip` writes `./ctw-native.zip` for WordPress upload. `media fetch` / `media sync` pull https images into `./media`. `products add` appends up to 4 dummy WooCommerce products. `init` also writes a starter `ctw-package.json` and `media/` (add `--woocommerce` for shop packages). `check` / `validate` verify schema plus CSS snippets (unclosed rules, HTML-escaped combinators) before `generate`.
